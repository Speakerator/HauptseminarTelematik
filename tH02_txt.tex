...
Since modern applications are no monolithic isolated constructs relying on their own resources but make use of many shared objects like additional libraries loaded at runtime, one of the key components of many systems is the dynamic loader. It resolves these dependencies, replaces references accordingly and loads the application and required shared objects into memory before the application is even started. These loaders usually are optimized for fast resolving and loading of said dependencies and often offer additional functionality for debugging or tracing purposes. However, this richness in functionality and the fact that all application, regardless of their privileges or possible remote accessibility make use of the same loader classify it as a prominent target of attacks. The fact that it is has to deal with several problems when used in a security related context worsen the case even more.
- explain the problems of the standard loader: privilege exploitation, circumventing the sandbox by loading packages prior to the initialisation of the sandbox itself, treatment as a black box from a sandboxing point of view, simply translated and used within the sandboxed application domain. \cite{t02_Payer}[S.21]

These problems are addressed by Payer et al., who developed a trusted runtime environment (TRuE), that combines a customized "`safe"' loader with a sandbox within the same domain to provide a protected foundation for application execution.
