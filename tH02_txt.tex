...
Payer et al. try to address these problem by introducing a trusted runtime environment (TRuE), that combines a customized "`safe"' loader with a sandbox within the same domain to provide a protected foundation for application execution. In contrast to a conventional sandbox, where the standard loader itself, the application and all required libraries are placed within the sandbox, TRuE positions the sandbox and the alternate secure loader in the same privileged domain while the untrusted application code as well as all additionally required libraries reside in the lower-privileged and controlled application domain. 
