...
The final type model with the \textit{Widget} type as its core was then used to verify the behaviour of the adsafe.js runtime library. It prove to be effective, since Politz et al. discovered several new bugs in ADsafe. However, there are some drawbacks of their solution that need mentioning. First of all, it was impossible to apply their verification approach to ADsafe without any refactoring of the runtime libraries. These were necessary because the library implemented programming patterns they were not able to verify with their type system\cite{t02_ADsafety}[p. 9]. Since their type system is destined to be universally applicable to any JS-based sandbox solution, these possible restrictions further increase the required effort to apply their solution to existing systems.
...
still missing:
benefits:
automated testing, especially handy for frequently repeated testing e.g. when develloping an new sandbox, which is possible due to the short "runtime" 
