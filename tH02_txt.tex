...
Dewald et al. circumvent these issues and present a transparent solution that allows client-side real-time analysis of websites and allows detection of a wide range of exploits. At its core, it utilizes Mozilla's Spider Monkey JavaScript Engine to execute scripts retrieved from websites and extended it with a novel analysis framework which logs executed actions. Similar to most sandboxes, it implements methods for both static and dynamic analysis. Static analysis is performs common JavaScript code checks as well as an iFrame / same-origin analysis. The former implements several heuristics for detecting manipulation of an objects prototype or maleficent use of \textit{eval} or similar function. However, Dewald et al. discovered that many benign websites also use this functions, which is why this method is complemented with the dynamic analysis depicted later in this section. The latter inspects whether the web page contains suspicious iFrames that may be used by an attacker to compromise a website and load content from another website. An iFrame is considered suspicious when it is hidden, e.g. through size attributes equal to zero or a position outside of the visible area, and refers to a foreign domain \cite{t02_Dewald}[p. 3].

solution:
-client-side detection technique transparent to the user
- general / able to detect wide range of exploits
...
- implemented as a Dynamic Link Library which is either called from a Browser Helper Object (BHO) for internet explorer uppon site access or from a wrapper executable, which allows analysis of multiple websites with a more detailed report.
-standalone DLL makes it easier to develop similar plug-ins for other browsers (BHO only works with IE - similar to a plug-in, supported since version 4, has DOM access)
- 
