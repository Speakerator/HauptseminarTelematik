...
Politz et al. try to address this problem and present a strongly automated method for the verification of Java Script Sandboxes based on a novel type system which evaluates the sandboxes properties. They applied and verified their system to the AdSafe Sandbox. Before presenting their solution in more detail, aforementioned properties, which apply to a secure sandbox, shall be briefly presented. According to \cite{t02_ADsafety}, they list as follows:
...
The verification of a sandboxes using a type based approach yields a few benefits towards other approaches. First of all most developers are familiar with type systems, ensuring understanding of what the system says about their code, e.g. when not all properties are met. Secondly, verification based on predefined types is generally faster than evaluating a system via whole-program analysis. And finally, Politz et al. proof the soundness of their type system as part of their work, making it an effective tool for verification.

The developed type system models the behaviour and restrictions of the components, which the sandbox interacts with and that enforce security: the sandboxed application, the browser including the programming language itself as well as the reference monitors or [[wie heißt das Wort in den Kapitel vorher?]]. Their type-based representation will be discussed in the following section.
...
Their is no specification of the structure of an application, that may be encapsulated in a sandbox. However, every sandbox expects to execute against application that passed its static code checks. In the case of ADsafe, static checks are performed using JSLint. Politz et al. therefore developed a type \textit{Widget} that regards the type and shape of any value or expression that is passed between the sandboxed application and the sandbox. It reflects the checks performed by JSLint and therefore allows or blacklists expressions. The entire specification for the type Widget is displayed in figure \ref{tH02_widget_type}.
